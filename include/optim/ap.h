...
namespace ap
{
	...
	********************************************************************/
	...
	public:
		...
		template_1d_array(const T&val, int sz)
		{
			m_iVecSize = sz;
			m_Vec=new T[m_iVecSize];
			for (size_t k =0;k<sz;k++) m_Vec[k] = val;
			m_iLow = 1;
			m_iHigh = m_iVecSize;
		};
		...
		template_1d_array& operator+=(const template_1d_array &rhs){
			ap_error::make_assertion(m_iVecSize ==rhs.m_iVecSize);
			for (int k = 0;k<m_iVecSize;k++)
				m_Vec[k] += rhs.m_Vec[k];
			return *this;
		};
		template_1d_array& operator-=(const template_1d_array &rhs){
		...
		template_1d_array& operator+=(const T &val){
			for (int k = 0;k<m_iVecSize;k++)
				m_Vec[k] += val;
			return *this;
		};
...
//#endif
		...
		int size()const{return m_iVecSize;};
	...
	template<class T> template_1d_array<T>  operator+(const template_1d_array<T> &arg1, const template_1d_array<T> &arg2){
		template_1d_array<T> arg = arg1;
		arg+=arg2;
		return arg;
	};
	template<class T> template_1d_array<T>  operator-(const template_1d_array<T> &arg1, const template_1d_array<T> &arg2){
	...
	template<class T> template_1d_array<T>  operator+(const template_1d_array<T> &arg1, const T &arg2){
		template_1d_array<T> arg = arg1;
		arg+=arg2;
		return arg;
	};
	...
	template<class T> template_1d_array<T>  operator*(const T  &arg2,const template_1d_array<T> &arg1){
		template_1d_array<T> arg = arg1;
		arg*=arg2;
		return arg;
	};
	...
	********************************************************************/
	...
	public:
		...
		template_2d_array(const T&val, int rows,int cols)
		{
#ifndef NO_AP_ASSERT
			ap_error::make_assertion(rows>0 && cols>0);
#endif
			m_iVecSize = rows*cols;
			m_iAllocated = m_iVecSize;
			m_Vec = new T[m_iVecSize];
			m_iLow1 = 1;
			m_iLow2 = 1;
			m_iHigh1 = rows;
			...
			for (int k = 0;k<m_iVecSize;k++) m_Vec[k] = val;
		};

		template_2d_array(const template_2d_array &rhs)
		{
			m_iVecSize = rhs.m_iVecSize;
			m_iAllocated = rhs.m_iVecSize;
			m_iLow1 = rhs.m_iLow1;
			m_iLow2 = rhs.m_iLow2;
		...
		int size(int dim)const{
			if (m_iVecSize==0)
				return 0;
			if (dim == 1) return m_iHigh1-m_iLow1+1;
			if (dim == 2) return m_iHigh2-m_iLow2+1;
			return 0;
		};
		...
		template_2d_array &transp(){
// bad style???
			long m_iLow1tmp = m_iLow2;
			long m_iHigh1tmp = m_iHigh2;
			long m_iLow2tmp = m_iLow1;
			long m_iHigh2tmp = m_iHigh1;
			long m_iConstOffsettmp = -m_iLow2tmp-m_iLow1tmp*(m_iHigh2tmp-m_iLow2tmp+1);
			long m_iLinearMembertmp = (m_iHigh2tmp-m_iLow2tmp+1);
			long nrow = size(1), ncol = size(2);
			T *m_Vectmp = new T[m_iVecSize];
			for (int k = 0;k<m_iVecSize;k++){
				int row = k/ncol;
				int col = k%ncol;
				m_Vectmp[col*nrow+row ] = m_Vec[k];
			}
			m_iLow1 = m_iLow1tmp;
			m_iHigh1 = m_iHigh1tmp;
			m_iLow2 = m_iLow2tmp;
			m_iHigh2 = m_iHigh2tmp;
			m_iConstOffset = m_iConstOffsettmp;
			m_iLinearMember = m_iLinearMembertmp;
			delete[] m_Vec;
			m_Vec = m_Vectmp;
			m_iAllocated = m_iVecSize;
			return *this;
...
//--------------------------------------------------------------------------------------
		void erasecolumn(int ncol){
#ifndef NO_AP_ASSERT
			ap_error::make_assertion(ncol>=m_iLow2 && ncol<=m_iHigh2);
#endif

			int offset = 1,beg = ncol - m_iLow2;
			for(int k = beg+1; k < m_iVecSize; k++){
				if ( !( (k-beg) % (m_iHigh2-m_iLow2+1)) ){offset++; continue;}
				m_Vec[k-offset] = m_Vec[k];
			}
			m_iHigh2--;
			m_iConstOffset = -m_iLow2-m_iLow1*(m_iHigh2-m_iLow2+1);
			m_iLinearMember--;
			m_iVecSize -= (m_iHigh1 - m_iLow1+1);
		}
//--------------------------------------------------------------------------------------
		void eraserow(int nrow){
#ifndef NO_AP_ASSERT
			ap_error::make_assertion(nrow>=m_iLow1 && nrow<=m_iHigh1);
#endif
			int offset = m_iHigh2 - m_iLow2 + 1;
			if (nrow != m_iHigh1){ // !last row
				for(int k = (nrow-m_iLow1+1)*(m_iHigh2 - m_iLow2)+1; k < m_iVecSize; k++){
					m_Vec[k-offset] = m_Vec[k];
				}
			}
			m_iHigh1--;
			m_iVecSize -= (m_iHigh2 - m_iLow2+1);
		}
//--------------------------------------------------------------------------------------
		void insertcolumn(const template_1d_array<T> &vec,int ncol){
#ifndef NO_AP_ASSERT
			ap_error::make_assertion(ncol>=m_iLow2 && ncol<=m_iHigh2+1);
			ap_error::make_assertion(vec.size() == (m_iHigh1-m_iLow1+1));
#endif
			int vecsize = vec.size();

			if ( (m_iAllocated - m_iVecSize) < vecsize){ //must allocate additional mem
				m_iAllocated = m_iVecSize+vecsize;
				T *m_Vectmp = new T[m_iAllocated];
				int tpos = 0,vpos = vec.getlowbound(),k = 0, tpos2=0;
				for (;k<m_iVecSize+vecsize;k++){
					if ( k%(m_iHigh2-m_iLow2+2) == ncol-m_iLow2 )
						m_Vectmp[tpos++] = vec(vpos++);
					else
						m_Vectmp[tpos++] = m_Vec[tpos2++];
				}
				delete[] m_Vec;
				m_Vec = m_Vectmp;
			}
			else // there are enough mem allocated
			{
				int tpos = m_iVecSize + vecsize - 1,vpos = vec.gethighbound(),k = m_iVecSize+vecsize-1,
					tpos2=m_iVecSize-1;
				for (;k>=(ncol-m_iLow2);k--){
					if ( k%(m_iHigh2-m_iLow2+2) == (ncol-m_iLow2) )
						m_Vec[tpos--] = vec(vpos--);
					else
						m_Vec[tpos--] = m_Vec[tpos2--];
				}
			}
			m_iVecSize += vecsize;
			m_iHigh2++;
...
//--------------------------------------------------------------------------------------
		void insertrow(const template_1d_array<T> &vec,int nrow){
#ifndef NO_AP_ASSERT
			ap_error::make_assertion(nrow>=m_iLow1 && nrow<=m_iHigh1);
			ap_error::make_assertion(vec.size() == (m_iHigh2-m_iLow2+1));
#endif
			int offset = (m_iHigh2-m_iLow2+1)*(nrow-m_iLow1);
			int vecsize = vec.size();
			if ( (m_iAllocated - m_iVecSize) < vecsize){ //must allocate additional mem
				m_iAllocated = m_iVecSize+vecsize;
				T *m_Vectmp = new T[m_iAllocated];
				for (int k = 0;k<offset;k++) m_Vectmp[k] = m_Vec[k];
				for (int k = 0;k<vecsize;k++)
					m_Vectmp[k+offset] = vec(k+vec.getlowbound());
				for (int k = offset;k<m_iVecSize;k++)
					m_Vectmp[k+vec.size()] = m_Vec[k];
				delete[] m_Vec;
				m_Vec = m_Vectmp;
			}
			else // there are enough mem allocated
			{
				for (int k = m_iVecSize-1;k>=offset;k--)
					m_Vec[k+vecsize] = m_Vec[k];
				for (int k = vecsize-1;k>=0;k-- )
					m_Vec[offset + k] = vec(k+vec.getlowbound());
			}
			m_iVecSize += vecsize;
			m_iHigh1++;
		}
	private:
		...
		unsigned	m_iAllocated;
	};

	template<class T> template_1d_array<T>  operator*(const template_1d_array<T> &arg1, const template_2d_array<T> &arg2){
#ifndef NO_AP_ASSERT
		ap_error::make_assertion(arg1.size() == arg2.size(1));
#endif
		template_2d_array<T> res;
		res.setbounds(1,arg1.size(1),1,arg2.size(2));

		for (int r = 1;r<=arg1.size(1);r++){
			for (int c = 1;c<=arg2.size();c++)

				res(r,c) += arg1(c)*arg2(c,r);

		}
		return res;
	};

	template<class T> template_1d_array<T>  operator*(const template_2d_array<T> &arg1,const template_1d_array<T> &arg2){
#ifndef NO_AP_ASSERT
		ap_error::make_assertion(arg2.size() == arg1.size(2));
#endif
		template_1d_array<T> res(0,arg1.size(1));

		for (int r = 1;r<=res.size();r++){
			for (int c = 1;c<=arg1.size(2);c++)
				res(r) += arg1(r,c)*arg2(c);
		}
		return res;
	};

	template<class T> template_2d_array<T>  operator*(const template_2d_array<T> &arg1,const template_2d_array<T> &arg2){
#ifndef NO_AP_ASSERT
		ap_error::make_assertion(arg2.size(1) == arg1.size(2));
#endif
		template_2d_array<T> res(0,arg1.size(1),arg2.size(2));

		for (int r = 1;r<=res.size(1);r++){
			for (int c = 1;c<=res.size(2);c++){
				T val(0);
				for (int k = 1;k<=arg2.size(1);k++)
					val += arg1(r,k)*arg2(k,c);
				res(r,c) = val;
			}
		}
		return res;
