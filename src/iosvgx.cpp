...
RESCODE georis::SVGXWriter::saveObject(UID uid,ObjectType ot,const std::vector<double> &params,UID parent){
    switch (ot){
    case OT_POINT:
    {
        ...
        if (parent != NOUID)
            elem->SetAttribute("parentid",(unsigned)parent);
...
RESCODE georis::SVGXWriter::saveConstraint(UID uid, ConstraintType ct, const std::vector<UID> &constrobj, double *param){
    tinyxml2::XMLElement *elem = m_Doc.NewElement("constraint");

    if (elem == nullptr) return RC_RUNTIME_ERR;

    elem->SetAttribute("id",(unsigned)uid);
    elem->SetAttribute("type", (unsigned)ct);
    if (param != nullptr)
    ...
    for (auto it: constrobj){
        ...
        elco->SetAttribute("idref",(unsigned)(it));
...
RESCODE georis::SVGXReader::loadObject(UID &uid,ObjectType &ot,std::vector<double> &params, UID &parent){
    ...
    if ( strncmp("line",m_CurrentElem->Name(),4) == 0 ){
        ot = OT_SEGMENT;
        params.resize(4);
    ...
    if ( strncmp("circle",m_CurrentElem->Name(),6) == 0 ){
        ot = OT_CIRCLE;
        params.resize(3);
        if ( tinyxml2::XML_SUCCESS != m_CurrentElem->QueryDoubleAttribute("x", &(params[0])) ) return RC_RUNTIME_ERR;
        if ( tinyxml2::XML_SUCCESS != m_CurrentElem->QueryDoubleAttribute("y", &(params[1])) ) return RC_RUNTIME_ERR;
...
RESCODE georis::SVGXReader::loadConstraint(UID &uid,ConstraintType &ct, std::vector<UID>&, double &param){
